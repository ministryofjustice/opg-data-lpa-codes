	...
	runBoth(t, "create", func(t *testing.T, fn lambdaFn) {
		...
		if assert.Nil(t, err) {
			...
			if assert.Len(t, codes["codes"], 1) {
				...
				assertEqualEither(t,
					newIntWithDot(time.Now().AddDate(1, 0, 0).Unix()),
					newIntWithDot(time.Now().AddDate(1, 0, 0).Unix()-1),
					row.ExpiryDate)
...
func TestMarkUsed(t *testing.T) {
	runBoth(t, "mark used", func(t *testing.T, fn lambdaFn) {
		...
		if assert.Nil(t, err) {
			...
			assertEqualEither(t,
				newIntWithDot(seconds),
				newIntWithDot(seconds-1),
				row.ExpiryDate)
		}
	})
...
func newIntWithDot(i int64) intWithDot {
	return intWithDot(fmt.Sprintf("%d.0", i))
}
