module "lamdba_lpa_codes_v1" {
  ...
  package_type     = "Zip"
  runtime          = "python3.8"
  handler          = "app.lpa_codes.lambda_handler"
  dbstream_handler = "dynamodb_stream.lambda_handler"
  ecr_arn          = data.aws_ecr_repository.lpa_codes.arn
}

# New Lambda running on ECR images. Creating both to avoid downtime.
module "lpa_codes_lambda_v1" {
  source             = "./modules/lambda"
  environment        = local.environment
  aws_subnet_ids     = data.aws_subnet.private.*.id
  lambda_prefix      = "lpa-codes-image"
  logger_level       = "INFO"
  tags               = local.default_tags
  openapi_version    = "v1"
  rest_api           = aws_api_gateway_rest_api.lpa_codes
